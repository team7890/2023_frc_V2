...
public class ScoreConeTopGrabHorzBalance extends SequentialCommandGroup {

  // private boolean bAllianceColor = DriverStation.getAlliance();

  /** Creates a new AutoScoreConeTopAndMove. */
  public ScoreConeTopGrabHorzBalance(Arm_subsystem objArm, Forearm_subsystem objForearm, Wrist_subsystem objWrist, RollerHand_subsystem objRollerHand, Swerve_subsystem objSwerve) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
      new BingoBongoScoreConeTop(objArm, objForearm, objWrist).withTimeout(2.5),
      new ConeOuttake_command(objRollerHand).withTimeout(1.0),
      new RegFloorPickupHorzCone(objArm, objForearm, objWrist).withTimeout(1.0),
      new ParallelCommandGroup(
        new Swerve_auto(objSwerve, 0.3, 0.0, 0.0, false).withTimeout(8.0),
        new RegFloorPickupHorzCone(objArm, objForearm, objWrist).withTimeout(8.0),
        new ConeIntake_command(objRollerHand).withTimeout(8.0)
      ).withTimeout(4.5),
      new ParallelCommandGroup(
        new Swerve_balance(objSwerve, -0.45, 0.0, 0.0, false),
        new RegStowArm(objArm, objForearm, objWrist).withTimeout(8.0),
        new ConeIntake_command(objRollerHand).withTimeout(0.5)
      ).withTimeout(9.6)
      // new ParallelCommandGroup(
      //   new Swerve_auto(objSwerve, 0.3, 0.0, 0.0, false).withTimeout(1.8),
      //   new RegFloorPickupHorzCone(objArm, objForearm, objWrist).withTimeout(1.8),
