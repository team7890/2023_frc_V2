...
package frc.robot.commands.Button_commands.Unused;
...
public class xDoubleSubstationPickup extends CommandBase {
  ...
  double dArmTarget = -5.6; //
  double dForearmTarget = -46.7;  // 
  double dWristTarget = -77.2;  // 

  /** Creates a new ScoreCubeTop. */
  public xDoubleSubstationPickup(Arm_subsystem objArm_in, Forearm_subsystem objForearm_in, Wrist_subsystem objWrist_in) {
    objArm = objArm_in;
  ...
  public void execute() {
    switch (iState) {
      ...
      case 13:          // Move everything to stow targets        //TODO fix comment
        ...
        if (Math.abs(objForearm.getForearmAngle() - dForearmTarget) < 5.0 && Math.abs(objArm.getArmAngle() - dArmTarget) < 1.0) iState = 14;
        break;
      case 14:
        objArm.softStop();
